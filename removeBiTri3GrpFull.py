...
try:
    with open ('bigram_beauty_wordsFull.dat', 'rb') as fp:
            biscores_beauty = pickle.load(fp)
except FileNotFoundError:
    pass
       
try:        
    with open ('trigram_beauty_wordsFull.dat', 'rb') as fp:
            triscores_beauty = pickle.load(fp)
except FileNotFoundError:
    pass
        
try:
    with open ('bigram_fashion_wordsFull.dat', 'rb') as fp:
            biscores_fashion = pickle.load(fp)
except FileNotFoundError:
    pass

try:
    with open ('trigram_fashion_wordsFull.dat', 'rb') as fp:
            triscores_fashion = pickle.load(fp)
except FileNotFoundError:
    pass

try:        
    with open ('bigram_mobile_wordsFull.dat', 'rb') as fp:
            biscores_mobile = pickle.load(fp)
except FileNotFoundError:
    pass

try:
    with open ('trigram_mobile_wordsFull.dat', 'rb') as fp:
            triscores_mobile = pickle.load(fp)
except FileNotFoundError:
    pass
    

def removeBiTri3GrpFull(splits , typeDataset):
